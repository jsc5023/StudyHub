...
| email | VARCHAR(100) | UNIQUE, NOT NULL, INDEX idx_user_email | 이메일 (로그인 ID) |
...
| owner_id | BIGINT | FK → User(id) | 작성자 ID |
| created_at | DATETIME | DEFAULT CURRENT_TIMESTAMP | 생성일 |
| updated_at | DATETIME | ON UPDATE CURRENT_TIMESTAMP | 수정일 |
...
| user_id | BIGINT | FK → User(id) | 지원자 ID |
...
| created_at | DATETIME | DEFAULT CURRENT_TIMESTAMP | 생성일 |
| updated_at | DATETIME | ON UPDATE CURRENT_TIMESTAMP | 수정일 |
| UNIQUE(user_id, project_id) |  |  | 중복 지원 방지 |
...
| user_id | BIGINT | FK → User(id), INDEX idx_notification_user_read (user_id, is_read) | 수신자 ID |
...
### 🎯 UserTechStack (사용자 기술 스택 매핑)

| 필드명 | 타입 | 제약조건 | 설명 |
| --- | --- | --- | --- |
| user_id | BIGINT | FK → User(id), PK | 사용자 ID |
| tech_stack | VARCHAR(50) | PK | 기술 태그 |
...
| project_id | BIGINT | FK → Project(id) | 프로젝트 ID |
| created_at | DATETIME | DEFAULT CURRENT_TIMESTAMP | 생성일 |
| UNIQUE(user_id, project_id) |  |  | 중복 즐겨찾기 방지 |
...
| created_by | BIGINT | FK → User(id) | 생성자 ID |
| created_at | DATETIME | DEFAULT CURRENT_TIMESTAMP | 생성일 |
| updated_at | DATETIME | ON UPDATE CURRENT_TIMESTAMP | 수정일 |
...
| UNIQUE(project_id, user_id) |  |  | 프로젝트 중복 참여 방지 |
